...
class PriorityRuntime:
    ...
    MARKET_DATA_TIMEOUT_MARKERS = (
        '/market/candles', '/market/tickers', '/market/ticker',
        '/market/books', '/market/orderbook', '/market/trades',
    )
    ...
    def _is_market_data_timeout(cls, exc):
        text = str(exc).lower()
        timeout = ('requesttimeout' in type(exc).__name__.lower() or
                   'requesttimeout:' in text or 'request timeout' in text or 'timed out' in text)
        if not timeout:
            return False
        return '/market/' in text and any(marker in text for marker in cls.MARKET_DATA_TIMEOUT_MARKERS)

    @classmethod
    def _is_recoverable_data_halt_reason(cls, reason):
        reason = str(reason or '')
        if reason.startswith(cls.DATA_HALT_PREFIX):
            return True
        # Migration for HALTs persisted by builds before public market-data
        # RequestTimeout was routed through DATA_BLOCKED. This deliberately
        # reuses the narrow public-endpoint classifier: private/order/account/
    ...
    def _recover_running_data_halt(self):
        ...
        self._data_blocked_since = None
        self._alert_key = None
        self._notify('🟢 LIVE 데이터 HALT 자동복구', 심볼=self.engine.settings.symbol,
                     이전오류=reason, 확인='5m/15m 데이터 정상 · Bitget FLAT 2회 × 3연속 확인')
        return True
    ...
    def _healthy(self, status):
        ...
        if self._alert_key and str(self._alert_key).startswith('DATA_BLOCKED:'):
            ...
            self._alert_key = None
            self._notify('🟢 LIVE 데이터 장애 복구', 심볼=self.engine.settings.symbol,
                         상태=status, 이전오류=previous,
                         안내='Priority Runtime 데이터 수신이 정상화되었습니다.')

    def _halt_alert(self):
        reason = str(self.controller.state.get('halted') or 'unknown')
        self._alert_once('HALTED:' + reason, '🔴 LIVE Priority Runtime HALT',
                         심볼=self.engine.settings.symbol, 오류=reason,
                         owner=self.controller.state.get('owner'), pending=self.controller.state.get('pending'),
    ...
    def _data_blocked(self, reason, now):
        ...
        if elapsed >= self.DATA_ALERT_SECONDS:
            self._alert_once('DATA_BLOCKED:' + reason, '🟠 LIVE 데이터 연속 장애',
                             심볼=self.engine.settings.symbol, 오류=reason,
    ...
    def scan_once(self):
        now = self.clock()
        c = self.controller
        if self._is_recoverable_data_halt_reason(c.state.get('halted')):
    ...
    def snapshot(self):
        return dict(mode='PRIORITY_RUNTIME', status=self.last_status,
            ...
            halted=self.controller.state['halted'], profiles=self.controller.state['profile'],
            multipliers={tf: values['live_entry_multiplier'] for tf, values in PROFILES.items()},
            position=self.controller.state['position'], watermark=self.controller.state['watermark'])
