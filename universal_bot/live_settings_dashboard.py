...
def install_live_settings_dashboard(app, scanner) -> None:
    ...
    def save_live_settings(req: LiveSettingsUpdate):
        ...
        env_values = {
            ...
            'REQUIRE_EXCHANGE_PROTECTION': 'true' if req.require_exchange_protection else 'false',
            'LIVE_REQUIRE_ONE_WAY_MODE': 'true' if req.live_require_one_way_mode else 'false',
        }
        ...
        for runtime in scanner.runtimes:
            ...
            update = {
                ...
                'require_exchange_protection': bool(req.require_exchange_protection),
                'live_require_one_way_mode': bool(req.live_require_one_way_mode),
            }
...
_HTML = r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Elite LIVE Settings</title><style>*{box-sizing:border-box}body{margin:0;padding:14px;background:#0b0f14;color:#eef2f7;font-family:system-ui}.wrap{max-width:900px;margin:auto}.card{background:#17202c;border:1px solid #334155;border-radius:14px;padding:16px;margin:12px 0}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px}.f{display:flex;flex-direction:column;gap:6px;color:#9daabd;font-size:12px}input,button{padding:11px;border-radius:8px;border:1px solid #556579;background:#0b0f14;color:#eef2f7;font-size:14px}input[type=checkbox]{width:22px;height:22px}button{background:#2563a8;font-weight:800;cursor:pointer}.muted{color:#9daabd}.ok{color:#42e887}.bad{color:#ff6677}.warn{color:#ffcc66}.row{display:flex;flex-wrap:wrap;gap:8px;align-items:center}.metric{background:#0f1620;border-radius:9px;padding:10px}.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px}.big{font-size:20px;font-weight:800}</style></head><body><div class="wrap"><h1>Bitget Elite LIVE 설정</h1><div class="muted">실거래 주문 크기/레버리지 설정 · 총 진입 횟수에는 최초 진입이 포함됩니다.</div><div class="card"><div class="grid"><label class="f">계정 레버리지<input id="leverage" type="number" min="1" max="150"></label><label class="f">1회 진입 배수<input id="mult" type="number" min="0.1" max="50" step="0.1"></label><label class="f">총 진입 횟수<input id="entries" type="number" min="1" max="5"></label><label class="f">거래소 TP/SL 보호<input id="protect" type="checkbox"></label><label class="f">One-way 모드 요구<input id="oneway" type="checkbox"></label></div><div id="explain" class="muted" style="margin:12px 0"></div><div class="row"><button onclick="save()">저장 / 즉시 적용</button><button onclick="location.href='/strategy'">전략 설정</button><button onclick="location.href='/'">대시보드 홈</button></div><div id="status" style="margin-top:12px"></div></div><div class="card"><b>현재 LIVE 프로필</b><div id="metrics" class="metrics" style="margin-top:10px"></div><div id="runtime" class="muted" style="margin-top:10px"></div></div></div><script>let current={};function metric(n,v){return `<div class=metric><div class=muted>${n}</div><div class=big>${v}</div></div>`}function explain(){const m=Number(mult.value||0),e=Number(entries.value||0);document.getElementById('explain').textContent=`1회 ${m}배 × 총 ${e}회 = 최대 ${m*e}배 상한 · ${e===2?'최초 1회 + 추가 1회':`최초 1회 + 추가 ${Math.max(0,e-1)}회`}`;}async function load(){const r=await fetch('/api/live/settings'),d=await r.json();current=d;leverage.value=d.leverage;mult.value=d.live_entry_multiplier;entries.value=d.live_max_entries_per_position;protect.checked=!!d.require_exchange_protection;oneway.checked=!!d.live_require_one_way_mode;explain();metrics.innerHTML=metric('모드',d.bot_mode)+metric('프로필',d.execution_profile)+metric('마진',d.margin_mode)+metric('레버리지',d.leverage+'x')+metric('1회 진입',d.live_entry_multiplier+'x')+metric('총 진입',d.live_max_entries_per_position+'회')+metric('최대 상한',d.live_max_total_multiplier+'x');runtime.innerHTML=(d.runtimes||[]).map(x=>`${x.symbol}: ${x.mode} · 포지션 ${x.position_side} · 진입 ${x.entries}회 · 계정 레버리지 ${x.account_leverage==null?'-':x.account_leverage+'x'}`).join('<br>');}async function save(){status.textContent='저장 중...';status.className='warn';const body={leverage:Number(leverage.value),live_entry_multiplier:Number(mult.value),live_max_entries_per_position:Number(entries.value),require_exchange_protection:protect.checked,live_require_one_way_mode:oneway.checked};const r=await fetch('/api/live/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),d=await r.json();if(!r.ok){status.textContent='오류: '+(d.detail||'저장 실패');status.className='bad';return}status.textContent='저장 완료 · 런타임 즉시 적용됨';status.className='ok';await load();}mult.oninput=explain;entries.oninput=explain;load();</script></body></html>'''
