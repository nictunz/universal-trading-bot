...
class LiveSettingsUpdate(BaseModel):
    ...
    live_entry_multiplier: float = Field(gt=0, le=15)
    live_max_entries_per_position: int = Field(ge=1, le=2)
...
def install_live_settings_dashboard(app, scanner) -> None:
    ...
    def get_live_executions(
        symbol: str = 'BTC/USDT:USDT',
        limit: int = Query(100, ge=1, le=500),
    ):
        return {
            'symbol': symbol,
            'executions': history.list_executions(symbol=symbol, mode='LIVE', limit=limit),
        }
...
_HTML = r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Elite LIVE Settings</title><style>*{box-sizing:border-box}body{margin:0;padding:14px;background:#0b0f14;color:#eef2f7;font-family:system-ui}.wrap{max-width:1100px;margin:auto}.card{background:#17202c;border:1px solid #334155;border-radius:14px;padding:16px;margin:12px 0}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px}.f{display:flex;flex-direction:column;gap:6px;color:#9daabd;font-size:12px}input,button{padding:11px;border-radius:8px;border:1px solid #556579;background:#0b0f14;color:#eef2f7;font-size:14px}button{background:#2563a8;font-weight:800;cursor:pointer}.muted{color:#9daabd}.ok{color:#42e887}.bad{color:#ff6677}.warn{color:#ffcc66}.row{display:flex;flex-wrap:wrap;gap:8px;align-items:center}.metric{background:#0f1620;border-radius:9px;padding:10px}.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px}.big{font-size:20px;font-weight:800}table{width:100%;border-collapse:collapse;font-size:12px}th,td{padding:8px;border-bottom:1px solid #334155;text-align:right;white-space:nowrap}th:first-child,td:first-child{text-align:left}</style></head><body><div class="wrap"><h1>Bitget Elite LIVE 설정</h1><div class="muted">변경 가능: 계정 레버리지 · 1회 진입 배수 · 총 진입 횟수. 포지션 보유 중에는 변경이 차단되며, 거래소 TP/SL 보호와 One-way 요구는 안전상 고정됩니다.</div><div class="card"><div class="grid"><label class="f">계정 레버리지<input id="leverage" type="number" min="1" max="150"></label><label class="f">1회 진입 배수<input id="mult" type="number" min="0.1" max="15" step="0.1"></label><label class="f">총 진입 횟수<input id="entries" type="number" min="1" max="2"></label></div><div id="explain" class="muted" style="margin:12px 0"></div><div class="row"><button onclick="save()">저장 / 즉시 적용</button><button onclick="location.href='/strategy'">전략 설정</button><button onclick="location.href='/'">대시보드 홈</button></div><div id="status" style="margin-top:12px"></div></div><div class="card"><b>현재 LIVE 프로필</b><div id="metrics" class="metrics" style="margin-top:10px"></div><div id="runtime" class="muted" style="margin-top:10px"></div></div><div class="card"><b>LIVE 체결 품질</b><div class="muted" style="margin:5px 0 9px">목표·실체결 수량, 평균가, 슬리피지, 분할 횟수와 보호상태를 영구 기록합니다.</div><div style="overflow:auto;max-height:430px"><table><thead><tr><th>시각</th><th>상태</th><th>방향</th><th>목표</th><th>체결</th><th>체결률</th><th>평균가</th><th>슬리피지</th><th>분할</th></tr></thead><tbody id="executionRows"></tbody></table></div></div></div><script>let current={};function metric(n,v){return `<div class=metric><div class=muted>${n}</div><div class=big>${v}</div></div>`}function explain(){const m=Number(mult.value||0),e=Number(entries.value||0);document.getElementById('explain').textContent=`1회 ${m}배(최대 15배) × 총 ${e}회(최대 2회) = 최대 ${m*e}배 상한 · ${e===2?'최초 1회 + 추가 1회':`최초 1회 + 추가 ${Math.max(0,e-1)}회`}`;}async function loadExecutions(){const d=await(await fetch('/api/live/executions?symbol=BTC%2FUSDT%3AUSDT&limit=100')).json();executionRows.innerHTML=(d.executions||[]).map(x=>{const m=x.metadata||{},ratio=m.fill_ratio==null?'-':(Number(m.fill_ratio)*100).toFixed(1)+'%';return `<tr><td>${String(x.updated_at||'').slice(0,19)}</td><td>${x.status}</td><td>${x.side}</td><td>${Number(x.requested_qty||0).toFixed(6)}</td><td>${Number(x.filled_qty||0).toFixed(6)}</td><td>${ratio}</td><td>${Number(x.avg_fill_price||0).toFixed(2)}</td><td>${Number(x.slippage_percent||0).toFixed(4)}%</td><td>${x.child_orders||0}</td></tr>`}).join('')}async function load(){const r=await fetch('/api/live/settings'),d=await r.json();current=d;leverage.value=d.leverage;mult.value=d.live_entry_multiplier;entries.value=d.live_max_entries_per_position;explain();metrics.innerHTML=metric('모드',d.bot_mode)+metric('프로필',d.execution_profile)+metric('마진',d.margin_mode)+metric('레버리지',d.leverage+'x')+metric('1회 진입',d.live_entry_multiplier+'x')+metric('총 진입',d.live_max_entries_per_position+'회')+metric('최대 상한',d.live_max_total_multiplier+'x')+metric('TP/SL 보호',d.require_exchange_protection?'고정 ON':'OFF')+metric('One-way',d.live_require_one_way_mode?'고정 요구':'OFF')+metric('Discord',d.discord_notifications_enabled&&d.discord_configured?'ON':'OFF');runtime.innerHTML=(d.runtimes||[]).map(x=>`${x.symbol}: ${x.mode} · 포지션 ${x.position_side} · 진입 ${x.entries}회 · 계정 레버리지 ${x.account_leverage==null?'-':x.account_leverage+'x'}`).join('<br>');await loadExecutions()}async function save(){status.textContent='저장 중...';status.className='warn';const body={leverage:Number(leverage.value),live_entry_multiplier:Number(mult.value),live_max_entries_per_position:Number(entries.value)};const r=await fetch('/api/live/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),d=await r.json();if(!r.ok){status.textContent='오류: '+(d.detail||'저장 실패');status.className='bad';return}status.textContent='저장 완료 · 런타임 즉시 적용됨';status.className='ok';await load();}mult.oninput=explain;entries.oninput=explain;load();setInterval(loadExecutions,10000);</script></body></html>'''
