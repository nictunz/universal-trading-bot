...
<div class='bar'><b>백테스트 / 과거 데이터</b><div class='muted' style='margin-top:4px'>4개 선물거래소 정규화 거래량 + 수수료/슬리피지 반영 순수익 기준</div><div class='row' style='margin-top:9px'><input id='symbol' value='ETH/USDT:USDT' placeholder='심볼'><select id='asset'><option value='crypto'>Crypto</option><option value='stock'>Stock</option><option value='etf'>ETF</option></select><input id='exchange' value='bitget' placeholder='거래소'><select id='tf'><option>5m</option><option>15m</option><option>1h</option><option>4h</option><option>1d</option></select></div><div class='row' style='margin-top:8px'><label>시작 <input id='start' type='date'></label><label>종료 <input id='end' type='date'></label><button onclick='runBT()'>데이터 수집 + 백테스트</button><span id='status' class='status muted'></span></div></div>
