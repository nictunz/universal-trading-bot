...
def create_dashboard(scanner) -> FastAPI:
    ...
    def candles(
        ...
        try:
            return dashboard_data.candles(
                symbol=symbol, exchange=exchange, timeframe=timeframe, asset_class=asset_class,
    ...
    @app.get("/api/trades")
    def trades(symbol: str, mode: str = "ALL", start: str | None = None, end: str | None = None, limit: int = Query(2000, ge=1, le=10000)):
        try:
...
<div class='bar'>
<b>시장 차트 / 거래 기록</b><div class='muted' style='margin-top:4px'>저장된 거래소 OHLCV 캐시와 BACKTEST · PAPER · LIVE 기록을 한 화면에서 조회</div>
...
function drawMarket(){const c=document.getElementById('marketChart'),dpr=devicePixelRatio||1,w=c.clientWidth||1000,h=c.clientHeight||430;c.width=w*dpr;c.height=h*dpr;const x=c.getContext('2d');x.setTransform(dpr,0,0,dpr,0,0);x.clearRect(0,0,w,h);const data=lastCandles;if(!data.length){x.fillStyle='#9daabd';x.font='14px system-ui';x.fillText('저장된 차트 데이터가 없습니다.',18,30);return}const padL=58,padR=18,padT=18,padB=34,chartH=h-padT-padB;const lows=data.map(a=>+a.low),highs=data.map(a=>+a.high),lo=Math.min(...lows),hi=Math.max(...highs),span=Math.max(1e-9,hi-lo);const py=v=>padT+(hi-v)/span*chartH,step=(w-padL-padR)/Math.max(1,data.length),body=Math.max(1,Math.min(7,step*.65));x.strokeStyle='#263445';x.fillStyle='#9daabd';x.font='11px system-ui';for(let k=0;k<=4;k++){const y=padT+chartH*k/4,p=hi-span*k/4;x.beginPath();x.moveTo(padL,y);x.lineTo(w-padR,y);x.stroke();x.fillText(p.toFixed(p>100?1:3),3,y+4)}data.forEach((a,i)=>{const cx=padL+(i+.5)*step,up=+a.close>=+a.open,col=up?'#42e887':'#ff6677';x.strokeStyle=col;x.fillStyle=col;x.beginPath();x.moveTo(cx,py(+a.high));x.lineTo(cx,py(+a.low));x.stroke();const yo=py(+a.open),yc=py(+a.close);x.fillRect(cx-body/2,Math.min(yo,yc),body,Math.max(1,Math.abs(yc-yo))) });const first=Date.parse(data[0].timestamp),last=Date.parse(data[data.length-1].timestamp),range=Math.max(1,last-first);function mx(t){return padL+(Date.parse(t)-first)/range*(w-padL-padR)}lastHistory.forEach(t=>{const et=t.entry_time,xt=t.exit_time;if(et){const xx=mx(et);if(xx>=padL&&xx<=w-padR){const yy=py(+(t.avg_entry_price||t.entry_price||lo));x.fillStyle=t.side==='LONG'?'#42e887':'#ff6677';x.beginPath();if(t.side==='LONG'){x.moveTo(xx,yy-8);x.lineTo(xx-5,yy+2);x.lineTo(xx+5,yy+2)}else{x.moveTo(xx,yy+8);x.lineTo(xx-5,yy-2);x.lineTo(xx+5,yy-2)}x.closePath();x.fill()}}if(xt){const xx=mx(xt);if(xx>=padL&&xx<=w-padR){const yy=py(+(t.exit_price||lo));x.fillStyle=t.reason==='TP'?'#42e887':t.reason==='SL'?'#ffcc66':'#eef2f7';x.beginPath();x.arc(xx,yy,4,0,Math.PI*2);x.fill()}}});x.fillStyle='#9daabd';x.fillText(new Date(first).toLocaleDateString(),padL,h-9);const label=new Date(last).toLocaleDateString(),tw=x.measureText(label).width;x.fillText(label,w-padR-tw,h-9)}
async function loadMarket(){const s=document.getElementById('chartStatus');s.textContent='차트/기록 로딩...';const q=new URLSearchParams({symbol:document.getElementById('symbol').value,exchange:document.getElementById('exchange').value,timeframe:document.getElementById('tf').value,asset_class:document.getElementById('asset').value,start:document.getElementById('start').value,end:document.getElementById('end').value,max_points:'1400'});const tq=new URLSearchParams({symbol:document.getElementById('symbol').value,mode:document.getElementById('mode').value,start:document.getElementById('start').value,end:document.getElementById('end').value,limit:'3000'});try{const [cr,tr]=await Promise.all([fetch('/api/candles?'+q),fetch('/api/trades?'+tq)]),cd=await cr.json(),td=await tr.json();if(!cr.ok)throw Error(cd.detail||'차트 오류');if(!tr.ok)throw Error(td.detail||'거래기록 오류');lastCandles=cd.candles||[];lastHistory=td.trades||[];drawMarket();const sm=td.summary||{},pf=sm.profit_factor==null?'-':Number(sm.profit_factor).toFixed(2);document.getElementById('historyMetrics').innerHTML=metric('거래',sm.trades||0)+metric('승리',sm.wins||0)+metric('승률',Number(sm.win_rate||0).toFixed(2)+'%')+metric('누적 순손익',Number(sm.pnl||0).toFixed(2),Number(sm.pnl)>=0?'good':'bad')+metric('PF',pf)+metric('차트 봉',cd.points||0);document.getElementById('historyTrades').innerHTML=lastHistory.map(t=>`<tr><td>${esc(t.mode)}</td><td>${esc(t.entry_time||'-')}</td><td>${esc(t.exit_time||'-')}</td><td>${esc(t.side)}</td><td>${Number(t.avg_entry_price||t.entry_price||0).toFixed(4)}</td><td>${Number(t.exit_price||0).toFixed(4)}</td><td>${Number(t.qty||0).toFixed(6)}</td><td class='warn'>${Number(t.cost||0).toFixed(2)}</td><td class='${Number(t.pnl)>=0?'good':'bad'}'>${Number(t.pnl||0).toFixed(2)}</td><td>${Number(t.pnl_percent||0).toFixed(3)}%</td><td>${esc(t.reason||'')}</td></tr>`).join('');s.textContent=`${cd.exchange} ${cd.timeframe} · ${cd.points||0}봉${cd.sampled?' · 화면 샘플링':''} · 거래 ${sm.trades||0}건`;s.className='status ok'}catch(e){s.textContent='오류: '+e.message;s.className='status error';lastCandles=[];drawMarket()}}
...
setDefaultDates();refreshHealth();refreshReadiness();refresh();loadMarket();setInterval(refresh,2000);setInterval(refreshHealth,10000);window.addEventListener('resize',()=>{drawEquity(lastEquity);drawMarket()});
