...
def _one(runtime):
    ...
    p = state.position
    return {"symbol": state.symbol, "timeframe": state.timeframe, "timestamp": state.timestamp.isoformat() if state.timestamp else None, "position": {"side": p.side, "size": p.size, "entry": p.entry_price, "tp": p.tp, "sl": p.sl, "entries": p.entries}, "signal": state.signal.side if state.signal else None, "signal_reason": state.signal.reason if state.signal else None, "values": state.values, "stats": state.stats, "error": runtime.last_error, "live_safety": {"enabled": safety.enabled, "halted": safety.halted, "reason": safety.reason, "consecutive_errors": safety.consecutive_errors, "protection_ok": safety.protection_ok, "last_reconciliation": safety.last_reconciliation.isoformat() if safety.last_reconciliation else None, "last_data": safety.last_data.isoformat() if safety.last_data else None, "exchange_position": safety.exchange_position, "internal_position": safety.internal_position}}
...
def create_dashboard(scanner) -> FastAPI:
    ...
    def health():
        ...
        return {"status": "halted" if halted else ("degraded" if errors else "ok"), "strategy": "Volume Strategy FINAL Universal v15", "symbols": len(runtimes), "errors": errors, "live_halted": halted}
