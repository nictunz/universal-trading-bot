...
def payload(tf, now, volume):
    ...
    return {
        ...
        'markets': {
            'BTC/USDT:USDT': {
                ex: [[boundary - delta, volume], [boundary, 9999]]
...
def test_dual_native_volume_keeps_legacy_and_filters_open_bar(tmp_path):
    ...
    assert relay.latest_common_timestamp('BTCUSDT', '5m') == relay.fetch_volume(
        'binance', 'BTCUSDT', '5m'
    ).index[-1]
...
def test_missing_five_does_not_break_fifteen(tmp_path):
    relay, main, sidecar, now = files(tmp_path)
    sidecar.unlink()
    with pytest.raises(RuntimeError, match='mobile relay snapshot not found'):
        relay.fetch_volume('binance', 'BTCUSDT', '5m')
    assert relay.fetch_volume('binance', 'BTCUSDT', '15m').tolist() == [150]


def test_reader_retries_transient_atomic_replace_gap(tmp_path, monkeypatch):
    relay, main, sidecar, now = files(tmp_path)
    original = Path.read_text
    attempts = {'count': 0}

    def flaky_read_text(path, *args, **kwargs):
        if path == sidecar and attempts['count'] == 0:
            attempts['count'] += 1
            raise FileNotFoundError(path)
        return original(path, *args, **kwargs)

    monkeypatch.setattr(Path, 'read_text', flaky_read_text)
    assert relay.fetch_volume('binance', 'BTCUSDT', '5m').tolist() == [50]
...
def test_bad_sidecar_cannot_masquerade_as_fresh_five(tmp_path, fault):
    relay, main, sidecar, now = files(tmp_path)
    if fault == 'stale':
        sidecar.write_text(json.dumps(payload('5m', now - 180000, 50)))
    elif fault == 'wrong_timeframe':
        sidecar.write_text(json.dumps(payload('15m', now, 150)))
    else:
