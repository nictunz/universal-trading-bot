...
public class LocalMarketChartActivity extends Activity implements CandleChartView.Navigator {
    private static final Set<String> ACTIVE_APPLIES=ConcurrentHashMap.newKeySet();
    ...
    private boolean blocked() {
        if(busy || !ACTIVE_APPLIES.isEmpty() || BacktestForegroundService.isWorkerRunning()){
    ...
    @Override public void onCreate(Bundle state){
        ...
        warning.setOnClickListener(v->showStrategyIssueOrQuality());
    ...
    private void applyStrategy(String parameters){
        ...
        final String activeKey=normalizedPath(m[0]);
        ACTIVE_APPLIES.add(activeKey);
        prefs().edit()
        ...
        worker.execute(()->{try{
            ...
            prefs().edit()
                ...
                .apply();
            runOnUiThread(()->{busy=false;if(disposed)return;progress.dismiss();selector.setEnabled(true);databaseSelector.setEnabled(true);resultPath=saved;loadedResult=applied;loadedResultPath=saved;loadedResultModified=savedModified;resultReadError="";status.setText("전략 적용 완료 · 차트 진단을 불러오는 중…");warning.setText("✅ 전략 적용 완료 · 봉별 진단을 표시합니다.");diagnostic.setText("전략 적용 완료 · 봉을 터치하면 실제 엔진 진단이 표시됩니다.");modeText.setText("DB 분석 · 전략 적용 완료");load();});
        }catch(Exception e){
            ...
            runOnUiThread(()->{busy=false;if(disposed)return;progress.dismiss();selector.setEnabled(true);databaseSelector.setEnabled(true);status.setText("전략 적용 실패 · 다시 시도하세요.");warning.setText("⚠ 전략 적용 실패: "+failure);diagnostic.setText("전략 적용 실패 · DB 봉 수와 데이터 검사를 확인하세요.");modeText.setText("DB 분석 · 전략 적용 실패");});}
        finally{ACTIVE_APPLIES.remove(activeKey);cancellation.delete();}});
    }
    ...
    private void rsiSettings(){
        ...
        TextView hint=new TextView(this);hint.setText("차트 RSI 표시 설정입니다. 진입 조건 변경은 전략을 수정한 뒤 다시 적용하세요.");form.addView(hint);
        String[] labels={"RSI 길이 (2~500)","LONG 최소","LONG 최대","SHORT 최소","SHORT 최대"};
        double[] values={chartRsiLength,chartRsiLongMin,chartRsiLongMax,chartRsiShortMin,chartRsiShortMax};
        EditText[] fields=new EditText[5];
        for(int i=0;i<5;i++){TextView label=new TextView(this);label.setText(labels[i]);form.addView(label);fields[i]=new EditText(this);fields[i].setSingleLine(true);fields[i].setInputType(android.text.InputType.TYPE_CLASS_NUMBER|android.text.InputType.TYPE_NUMBER_FLAG_DECIMAL);fields[i].setText(i==0?Integer.toString(chartRsiLength):Double.toString(values[i]));form.addView(fields[i]);}
        ScrollView scroll=new ScrollView(this);scroll.addView(form);
        AlertDialog dialog=new AlertDialog.Builder(this).setTitle("RSI · 롱숏 진입 범위").setView(scroll).setPositiveButton("차트에 적용",null).setNeutralButton("적용 전략 값 불러오기",null).setNegativeButton("취소",null).create();
        dialog.setOnShowListener(v->{
            dialog.getButton(AlertDialog.BUTTON_NEUTRAL).setOnClickListener(w->{
                JSONObject params=activeSummary.optJSONObject("effective_parameters");
                if(params==null){Toast.makeText(this,"먼저 전략을 적용해 주세요",Toast.LENGTH_SHORT).show();return;}
                fields[0].setText(Integer.toString(params.optInt("rsi_length",10)));
                String[] keys={"rsi_oversold_min","rsi_oversold_max","rsi_overbought_min","rsi_overbought_max"};
                for(int i=0;i<4;i++)fields[i+1].setText(Double.toString(params.optDouble(keys[i],values[i+1])));
            });
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(w->{try{
                int length=Integer.parseInt(fields[0].getText().toString());double a=Double.parseDouble(fields[1].getText().toString()),b=Double.parseDouble(fields[2].getText().toString()),c=Double.parseDouble(fields[3].getText().toString()),d=Double.parseDouble(fields[4].getText().toString());
                if(length<2||length>500||!(0<=a&&a<=b&&b<c&&c<=d&&d<=100))throw new IllegalArgumentException();
                chartRsiLength=length;chartRsiLongMin=a;chartRsiLongMax=b;chartRsiShortMin=c;chartRsiShortMax=d;indicatorPane=1;saveWorkspace();dialog.dismiss();load();
        ...
        new AlertDialog.Builder(this).setTitle("차트 설정")
            .setItems(new String[]{fullscreen?"전체화면 해제":"전체화면","EMA 20/60/200 "+(ema?"끄기":"켜기"),"볼린저밴드 20·2σ "+(bands?"끄기":"켜기"),"보조지표 선택","전체 범위 600봉","상세 진단","RSI 길이·롱숏 범위 설정"},(d,i)->{
                ...
                if(i==3)new AlertDialog.Builder(this).setTitle("가격 차트 아래 보조창")
                    .setSingleChoiceItems(new String[]{"거래량만","RSI("+chartRsiLength+") · 롱숏 범위","ADX(14) 참고지표","전략 정규화 거래량"},indicatorPane,(a,k)->{indicatorPane=k;chart.options(ema,bands,indicatorPane);saveWorkspace();a.dismiss();}).show();
     ...
     */
    private void showStrategyIssueOrQuality(){
        String failure=prefs().getString("chart_apply_error:"+selectedDatabasePath,"");
        if(failure.isEmpty())failure=resultReadError;
        if(failure.isEmpty()){showDataQuality();return;}
        final String detail=failure;
        new AlertDialog.Builder(this).setTitle("전략 적용 오류 상세")
            .setMessage(detail)
            .setPositiveButton("전략 다시 적용",(d,i)->strategyDialog())
            .setNeutralButton("오류 복사",(d,i)->{
                android.content.ClipboardManager clipboard=(android.content.ClipboardManager)getSystemService(CLIPBOARD_SERVICE);
                clipboard.setPrimaryClip(android.content.ClipData.newPlainText("전략 적용 오류",detail));
    ...
    private void checkStrategyApplyStatus(){
        ...
        String failure=prefs().getString("chart_apply_error:"+selectedDatabasePath,"");
        if("RUNNING".equals(state)&&!ACTIVE_APPLIES.contains(normalizedPath(selectedDatabasePath))){
            // A persisted flag is not proof that a worker survived process death.
            state="ERROR";
            failure="계산 작업이 중단되었습니다. 전략 적용을 다시 실행하세요. 기존 DB와 저장된 결과는 유지됩니다.";
            prefs().edit().putString("chart_apply_status:"+selectedDatabasePath,state)
                .putString("chart_apply_error:"+selectedDatabasePath,failure).apply();
        }
        ...
        if(saved.isEmpty()||!new File(saved).isFile()){
            prefs().edit().putString("chart_apply_status:"+selectedDatabasePath,"ERROR")
                .putString("chart_apply_error:"+selectedDatabasePath,"저장된 결과 파일을 찾을 수 없습니다. 전략을 다시 적용하세요.").apply();
            return;
        }
    ...
    @Override protected void onDestroy(){
        ...
        generation.incrementAndGet();
        // Drain queued tasks so their cancellation cleanup always releases ACTIVE_APPLIES.
        worker.shutdown();
        super.onDestroy();
    }
}
